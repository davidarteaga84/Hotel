...
class reserva {
	...
	public function setDataEntrada($valor){
		if ($valor!=""){
			...
			if (checkdate($mes,$dia,$any)) {
				$date=$any."-".$mes."-".$dia;
				if((($any*10000)+($mes*100)+$dia) >= date("Ymd")) {	$this->dataEntrada = $date; }
				else { $this->dataEntrada = date("Y-m-d"); }
			} else { $this->dataEntrada = date("Y-m-d"); }
		}
	}
	...
	public function obtenirDadesClient($nifClient){				//Martí 12/03/2014
		$reservaDB = new reserva_db();
		return $reservaDB->obtenirDadesClient($nifClient,$this);
	}
